      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-2 sm:gap-4">
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <span className="font-heading font-black text-lg sm:text-2xl tracking-tight text-stone-900 group-hover:text-red-600 transition-colors">
                  Frosty's
                </span>
                <span
                  className={`inline-block w-2 h-2 rounded-full ${
                    status.isOpen ? 'bg-blue-500 animate-pulse' : 'bg-amber-400'
                  }`}
                  title={status.isOpen ? 'Open Now (4 PM - 2 AM)' : 'Opens at 4 PM'}
                ></span>
              </div>
              <span className="text-[10px] font-extrabold uppercase tracking-wider text-red-600 block -mt-0.5 sm:-mt-1">
                Ice Cream & Treats • Green City
              </span>
            </div>
          </a>

          {/* Desktop Nav Links */}
          <nav className="hidden lg:flex items-center gap-6 text-sm font-bold text-stone-600" id="desktop-nav">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            >
              <i className="fa-solid fa-phone text-xs text-red-600"></i>
              <span className="hidden lg:inline">{STORE_INFO.phone}</span>
            </a>
            ...
            <button
              ...
              className="px-3 sm:px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-xs sm:text-sm shadow-xs transition-all flex items-center gap-2 cursor-pointer hover:scale-105 active:scale-95"
            >
              <i className="fa-solid fa-cart-shopping text-xs sm:text-sm"></i>
              <span className="hidden sm:inline">Cart</span>
              {cartCount > 0 && (
                <span className="bg-white text-red-700 font-extrabold text-[11px] px-1.5 py-0.2 rounded-full min-w-5 text-center shadow-xs">
                  {cartCount}
                </span>
              )}
            </button>
      ...
      {mobileMenuOpen && (
        <div className="bg-white border-b border-stone-200 px-4 pt-3 pb-5 space-y-3 shadow-lg animate-fadeIn">
          <div className="flex items-center justify-between pb-2 border-b border-stone-100 text-xs">
            ...
            <span
              className={`font-bold px-2 py-0.5 rounded-full text-[11px] ${
                status.isOpen ? 'text-blue-700 bg-blue-50' : 'text-amber-700 bg-amber-50'
              }`}
            >
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            {navLinks.map((link) => (
              <a
                ...
                className="px-3 py-2 rounded-xl bg-stone-50 hover:bg-blue-50 text-stone-800 hover:text-blue-700 text-xs font-bold flex items-center gap-2 transition-colors"
              >
          ...
          <div className="pt-2 border-t border-stone-100 flex items-center gap-2">
            <a
              href={`tel:${STORE_INFO.phone}`}
              className="flex-1 py-2.5 px-3 rounded-xl bg-red-50 hover:bg-red-100 text-red-800 text-xs font-bold flex items-center justify-center gap-2 border border-red-200"
            >
